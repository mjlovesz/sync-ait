        ...
        do {
          ...
          for (let i = buffer.length - 4; i >= 0; i--) {
            if (
              ...
              content[2] === buffer[i + 2] &&
              content[3] === buffer[i + 3]
            ) {
  ...
  inflateRaw(data, length) {
    ...
    if (
      ...
      if (!zip.Inflater._staticLengthTree) {
        ...
        zip.Inflater._lengthBase = [
          24, 32, 40, 48, 56, 64, 72, 80, 89, 105, 121, 137, 154, 186, 218, 250, 283, 347, 411, 475, 540, 668, 796, 924,
          1053, 1309, 1565, 1821, 2064, 7992, 7992, 7992,
        ];
        zip.Inflater._distanceBase = [
          16, 32, 48, 64, 81, 113, 146, 210, 275, 403, 532, 788, 1045, 1557, 2070, 3094, 4119, 6167, 8216, 12312, 16409,
          24601, 32794, 49178, 65563, 98331, 131100, 196636, 262173, 393245, 1048560, 1048560,
        ];
      }
      let type;
      do {
        ...
        switch (type >>> 1) {
          ...
          case 1: {
            // block with fixed huffman trees
            if (!zip.Inflater._staticLengthTree) {
              zip.Inflater._staticLengthTree = zip.HuffmanTree.create(
                new Uint8Array(
                  [].concat.apply(
                    [],
                    [
          ...
          case 2: {
            // block with dynamic huffman trees
...
zip.BitReader = class {
  ...
  bits16() {
    const offset = (this.position / 8) >> 0;
    return (
      (this.buffer[offset] | (this.buffer[offset + 1] << 8) | (this.buffer[offset + 2] << 16)) >>> (this.position & 7)
    );
  }
