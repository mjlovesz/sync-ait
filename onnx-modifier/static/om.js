...
function formatEnumToString(i) {
  const FORMAT_LIST = [
    ...
    'Fractal-Deconv',
    'C1HWNC0',
    'Fractal-Deconv-Transposed',
    'Fractal-Deconv-SP-Stride-Trans',
    'NC1HWC0(C04)',
    'Fractal-Z(C04)',
    'CHWN',
    'Fractal-Deconv-SP-Stride8-Trans',
    'HWCN',
    'NC1KHKWHWC0',
    'BN',
    'Filter HWCK',
...
om.Metadata = class {
  static open(context) {
    ...
    return context
      .request('./om-metadata.json', 'utf-8', null)
      .then((data) => {
  ...
  constructor(metadata, op, graph, weight, model) {
    ...
    for (let i = 0; i < op.input.length; ++i) {
      ...
      if (isNodeConst(inputNode)) {
        ...
        if (!weightDims) {
          //Save the first const as weight
        ...
        let tmpTensorType = new om.TensorType(
          inputDtype,
          inputDims,
          inputFormat,
          inputNode.attr['value'].t.desc.layout,
          dataLength
        );
    ...
    if (attrMeta && attrMeta.type) {
      ...
      if (attrMeta.type == 'Format') {
        const FORMAT_LIST = [
          ...
          'Fractal-Z(C04)',
          'CHWN',
          'Fractal-Deconc-SP-Stride8-Trans',
          'HWCN',
          'NC1KHKWHWC0',
          'Fractal-Deconv-SP-Stride-Trans',
          'BN',
  ...
  get rawShape() {
    ...
    if (this._dtype == 'float16') {
      ...
      if (this._denotation == 'NC1HWC0') {
        ...
        return new om.TensorShape([
          ~~((parseInt(dims[1]) + 15) / 16),
          dims[2],
          dims[3],
          ~~((parseInt(dims[0]) + 15) / 16) * 16,
          16,
        ]);
      }
    }
