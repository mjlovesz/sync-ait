...
## 介绍
AIT(Ascend Inference Tools)作为昇腾统一推理工具，提供客户一体化开发工具，支持一站式调试调优，当前包括debug、transplt、profile、analyze,benchmark等组件。
...
# 安装ait，包括debug、benchmark组件
./install.sh

# 重新安装ait及其debug、benchmark组件
...
其中，```<TASK>```为任务类型，当前支持debug、profile、analyze、benchmark，后续可能会新增其他任务类型，可以通过如下方式查看当前支持的任务列表：
```bash
