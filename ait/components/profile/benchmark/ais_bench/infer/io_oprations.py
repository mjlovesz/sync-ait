...
def get_narray_from_files_list(
    files_list,
    size,
    pure_data_type,
...
def get_tensor_from_files_list(
    ...
    ndata = get_narray_from_files_list(
...
#   Judge according to the realsize and file size of input 0. If the judgment fails,
...
def get_files_count_per_batch(intensors_desc, fileslist, no_combine_tensor_mode=False):
    ...
    if files_count_per_batch == 0:
        raise ZeroDivisionError('Invalid divisor `files_count_per_batch` == 0')
...
# Obtain tensor information and files information according to the input filelist.
# Create intensor form files list
# len(files_list) should equal len(intensors_desc)
def create_infileslist_from_fileslist(
    fileslist,
    intensors_desc,
    no_combine_tensor_mode=False
):
    ...
    for i, out in enumerate(outputs):
        ...
        if files_count_perbatch == 1 or ndata.shape[output_batchsize_axis] % files_count_perbatch == 0:
            ...
            logger.error('save out files error array shape:{} filesinfo:{} files_count_perbatch:{} ndata.shape{}:{}'.format(
                ndata.shape, infiles_paths, files_count_perbatch, output_batchsize_axis,
