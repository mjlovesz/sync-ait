...
def infer_simple():
    ...
    logging.info("outputs:{outputs} type:{type(outputs)}")

    logging.info("static infer avg:{np.mean(session.sumary().exec_time_list)} ms")
...
def infer_torch_tensor():
    ...
    logging.info("in torch tensor outputs[0].shape:{outputs[0].shape} type:{type(outputs)}")
    ...
    logging.info("in discontinuous torch tensor outputs[0].shape:{outputs[0].shape} type:{type(outputs)}")
...
def infer_dymshape():
    ...
    logging.info("inputs: custom_sizes: {output_size} outputs:{outputs} type:{type(outputs)}")

    # input args custom_sizes is list
    output_sizes = [100000]
    outputs = session.infer([ndata], mode, custom_sizes=output_sizes)
    logging.info("inputs: custom_sizes: {output_sizes} outputs:{outputs} type:{type(outputs)}")
    logging.info("dymshape infer avg:{np.mean(session.sumary().exec_time_list)} ms")
...
def infer_dymdims():
    ...
    logging.info("dymdims infer avg:{np.mean(session.sumary().exec_time_list))} ms")
