...
class FileStat:
    ...
    def is_basically_legal(self, perm='none', strict_read_permission=True):
        if sys.platform.startswith("win"):
            return self.check_windows_permission(perm)
        else:
            return self.check_linux_permission(perm, strict_read_permission=strict_read_permission)

    def check_linux_permission(self, perm='none', strict_read_permission=True):
        ...
        if not self.is_user_or_group_owner and self.is_exists:
            ...
            return False
        if perm == 'read':
            if strict_read_permission and self.permission & READ_FILE_NOT_PERMITTED_STAT > 0:
