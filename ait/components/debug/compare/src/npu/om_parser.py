...
from src.common.dynamic_argument_bean import DynamicArgumentEnum
...
DTYPE_OBJECT = "dtype"
DTYPE_MAP = {
             "DT_FLOAT": np.float32, "DT_FLOAT16": np.float16, "DT_DOUBLE": np.float64, "DT_INT8": np.int8,
...
class OmParser(object):
    ...
    def _is_input_shape_range(self):
        ...
        for attr in self.json_object.get(ATTR_OBJECT):
            if KEY_OBJECT in attr and attr.get(KEY_OBJECT) == ATC_CMDLINE_OBJECT:
                if VALUE_OBJECT in attr and S_OBJECT in attr.get(VALUE_OBJECT) and\
    ...
    def _get_range_shape_size_list(self, input_object):
        ...
        for attr in input_object.get(ATTR_OBJECT):
            if KEY_OBJECT in attr and attr.get(KEY_OBJECT) == SHAPE_RANGE_OBJECT and\
                VALUE_OBJECT in attr and attr.get(VALUE_OBJECT) and LIST_LIST_INT_OBJECT in attr.get(VALUE_OBJECT):
