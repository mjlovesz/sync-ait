...
│   │   ├-- {timestamp}_bin2npy      # npu dump数据转换为npy文件的数据目录，convert未选用情况下没有该目录
│   │   │   └-- 0                    # Device 设备 ID 号
│   │   │       └-- {om_model_name}  # 模型名称
│   │   │           └-- 1            # 模型 ID 号
│   │   │               ├-- 0        # 针对每个Task ID执行的次数维护一个序号，从0开始计数，该Task每dump一次数据，序号递增1
│   │   │               │   ├-- Add.8.5.1682067845380164.input.0.npy  #每个算子的输入npy文件，如果有多个输入，则会有多个npy文件
│   │   │               │   ├-- Add.8.5.1682067845380164.output.0.npy #每个算子的输出npy文件
│   │   │               │   ├-- ...
