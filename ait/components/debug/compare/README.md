...
- **不指定模型输入** 命令示例，**其中路径需使用绝对路径**
  ...
  - `-c，–-cann-path` (可选) 指定 `CANN` 包安装完后路径，不指定路径默认会从环境中寻找`CANN` 包路径，如果不存在则设为 `/usr/local/Ascend/ascend-toolkit/latest`
...
### 参数说明
  ...
  | -c，--cann-path           | CANN包安装完后路径，默认会从环境中寻找`CANN` 包路径，如果不存在则设为 `/usr/local/Ascend/ascend-toolkit/latest` | 否    |
