# Copyright 2023 Huawei Technologies Co., Ltd. 2023-2023. All rights reserved.
#
...
KnowledgeGroup.add_knowledge(Knowledge(
    "检查aclcreateEvent接口创建的event资源是否超过了1023，若超过了会创建失败",
    ['aclcreateEvent']
))
...
KnowledgeGroup.add_knowledge(Knowledge(
    ...
    ['CreateDvppApi', 'DvppCtl', 'DestroyDvppApi', 'DvppGetOutParameter', 'CreateVdecApi', 'VdecCtl', 'DestroyVdecApi',
        'CreateVenc', 'SetVencParam', 'RunVenc', 'DestroyVenc']
))
...
KnowledgeGroup.add_knowledge(Knowledge(
    "接口aclrtMallocHost在RC模型下需要替换成aclrtMalloc。",
    ['aclrtMallocHost']
))
...
KnowledgeGroup.add_knowledge(Knowledge(
    "接口aclrtFreeHost在RC模型下需要替换成aclrtFree。",
    ['aclrtFreeHost']
))
