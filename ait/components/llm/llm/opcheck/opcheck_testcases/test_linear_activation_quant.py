...
class ActivationGolden:
    ...
    def gelu_golden(in_tensors, _):
        ...
        try:
            float_result = 0.5 * float_in_tensors * (1 + torch.nn.functional.tanh(np.sqrt(2 / np.pi) * (float_in_tensors + 
                        0.044715 * torch.pow(float_in_tensors, 3))))
        except ZeroDivisionError as e:
            raise RuntimeError("Gelu golden: The divisor cannot be zero! Exception: {}".format(e))
    ...
    def fast_gelu_golden(in_tensors, _):
        ...
        try:
            ...
            raise RuntimeError("Fast gelu golden: The divisor cannot be zero! Exception: {}".format(e))
    ...
    def swish_golden(in_tensors, scale):
        ...
        try:
            ...
            raise RuntimeError("Swish golden: The divisor cannot be zero! Exception: {}".format(e))
        return float_result.half()
...
class TestLinearActivationQuantOperation(operation_test.OperationTest):
    ...
    def golden_calc(self, in_tensors):
        ...
        if self.op_param["hasBias"]:
            ...
            golden_result = golden_result.astype(np.int32) + in_tensors[2].astype(np.int32)
        golden_result = golden_result * intensor[3].astype(np.int64)
        golden_result = golden_result.astype(np.float16)
