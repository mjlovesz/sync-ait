...
def search_float_quant_matches(golden_path, my_path, golden_topo_json_path, my_topo_json_path):
    ...
    def type_based_matches(my_node, golden_node):
        ...
        for sub_node in my_node.children:
            if sub_node.node_type not in my_type_map:
                my_type_map[sub_node.node_type] = []
            my_type_map[sub_node.node_type].append(sub_node.node_name)
        for sub_node in golden_node.children:
            if sub_node.node_type not in golden_type_map:
                golden_type_map[sub_node.node_type] = []
            golden_type_map[sub_node.node_type].append(sub_node.node_name)
