...
@pytest.mark.parametrize("value", [-1, 3, 'string', None])
...
@pytest.mark.parametrize("value", ["Invalid|String", "Another<Invalid>String"])
...
@pytest.mark.parametrize("value", ["invalid_ids", "1,2,3", "1_2_3"])
...
@pytest.mark.parametrize("args", [None, [], ["arg1", "|", "arg3"]])
...
@pytest.mark.parametrize("command, expected", [("python3 aa.sh", True), ("invalid command", False)])
def test_check_exec_cmd(command, expected):
    assert check_exec_cmd(command) == expected


# Test cases for check_output_path_legality function
def test_check_output_path_legality_existing_path():
    with pytest.raises(ArgumentTypeError):
        check_output_path_legality("non_existing_output_path")
...
@pytest.mark.parametrize("value", ["invalid", "Yes", "No", "2", ""])
