...
3. 从工具解压目录下进入/ait/ait/benchmark/ais_bench目录下，执行如下命令进行编译：

   ```bash
   # 进入工具解压目录
   cd ${HOME}/ait/ait/benchmark
   # 构建aclruntime包
...
1. 执行requirements.txt文件中的依赖安装，执行如下命令：
   ...
   ```bash
<<<<<<< HEAD:ait/components/benchmark/README.md
   cd ${HOME}/ait/benchmark/
=======
   cd ${HOME}/ait/ait/profile/benchmark/
>>>>>>> 43f788332faa205af92dace38437eefa6724df41:ait/components/profile/benchmark/README.md
