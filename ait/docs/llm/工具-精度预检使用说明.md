...
部分算子在310IDUO上精度预检比对结果不可靠，是由于310IDUO上部分算子（如linearOperation、PagedAttentionOperationd等）存在原地读写的情况，导致部分输入数据成为“脏数据”，从而影响比对结果。

3. 为什么多卡算子运行报错？

涉及到多卡的算子（如AllGatherOperation、AllReduceOperation、LinearParallelOperation等）需要把所有卡上的tensor全部dump下来才能进行精度比对，否则会因为找不到相应路径而报RuntimeError。