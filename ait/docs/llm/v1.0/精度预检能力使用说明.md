...
- tensor信息会生成在默认落盘路径的atb_temp目录下，具体路径是 `{DUMP_DIR}/PID_TID`目录下。
...
## 3. 执行精度预检
### 3.1 使用示例
`ait llm opcheck -i {DUMP_DIR}/PID_TID/0/ -c {DUMP_DIR}/ait_dump/operation_io_tensors/{PID}/operation_tensors_{executeCount}.csv`
