...
| 参数名      | 说明                                                                           | 是否必选 |
| ----------- | ------------------------------------------------------------------------------ | -------- |
| token_id    | 用户手动输入，用于标识token的轮次                                              | 是       |
| data_id     | 用户手动输入，数据的**唯一标识**，用于与加速库侧数据匹配，以实现数据比对 | 是       |
| golden_data | 用户手动输入，需要dump的数据，格式为torch tensor                               | 是       |
| my_path     | 用户手动输入，需要dump数据对应的加速库侧.bin文件的路径                         | 是       |
| output_path | 用户手动输入，dump数据保存的路径                                               | 否       |
