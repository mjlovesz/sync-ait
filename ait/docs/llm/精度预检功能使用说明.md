...
| --custom-algorithms, -alg   | 指定自定义比对算法，格式应为“python_file_path.py:function”。自定义算法最好为独立文件，方法示例："def foo(golden_tensor, my_tensor): return float_value, string_message"，使用方式：-alg python_file_path.py:foo python_file_path:foo2| 否       |
