...
opcheck提供加速库内置算子的单算子精度预检能力。opcheck根据模型推理时dump的真实tensor数据以及算子param信息，通过构造相应的算子用例来计算标杆output，比较dump的算子output与标杆output的误差，以检测加速库算子精度是否达标。若使用旧版ait llm，请参考[精度预检能力使用说明](./v1.0/精度预检能力使用说明.md)。
...
- 从 whl 包安装 ait_llm 时或安装时未配置 `atb` 以及 `mindie-atb-models` 执行环境，需要手动编译 `libopchecker.so`
  - 首先，进入llm安装目录, 安装目录可通过`python3 -c 'import ait_llm, os; print(os.path.dirname(os.path.abspath(ait_llm.__file__)))'`确定
  - 执行编译命令
    ```
...
- `ait llm dump`相关参数使用请参考[加速库DUMP功能使用说明](./加速库DUMP功能使用说明.md)
...
判断公式可总结为
```
相对误差（rel_error）：rel_error = abs(actual_output - golden_output) / abs(golden_output)
实际的精度通过率（rel_precision_rate(%)）：rel_precision_rate = sum(rel_error <= etol) / size(rel_error)
