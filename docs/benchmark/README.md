# amit benchmark工具使用指南

## 简介
本文介绍benchmark推理工具，用来针对指定的推理模型运行推理程序，并能够测试推理模型的性能（包括吞吐率、时延）。

## 工具安装
- 工具安装请见 [amit一体化工具使用指南]()
- 单独安装benchmark工具请见[ais_bench工具使用指南]()
...
```bash
amit profile benchmark --model *.om
```
其中，*为OM离线模型文件名。
...
```bash
amit profile benchmark --model /home/model/resnet50_v1.om --output ./ --outfmt BIN --loop 5
```
...
```bash
amit profile benchmark --model /home/model/resnet50_v1.om --output ./ --debug 1
```
...
```bash
amit profile benchmark --model ./resnet50_v1_bs1_fp32.om --input "./1.bin,./2.bin,./3.bin,./4.bin,./5.bin"
```
...
```bash
amit profile benchmark --model ./resnet50_v1_bs1_fp32.om --input "./"
```
...
```bash
amit profile benchmark --model ./save/model/BERT_Base_SQuAD_BatchSize_1.om --input ./data/SQuAD1.1/input_ids,./data/SQuAD1.1/input_mask,./data/SQuAD1.1/segment_ids
```
...
```bash
amit profile benchmark --model ./pth_resnet50_bs1.om --input ./data/ --device 1,2
```
...
```bash
amit profile benchmark --model ./resnet50_v1_dynamicbatchsize_fp32.om --input=./data/ --dymBatch 2
```
...
```bash
amit profile benchmark --model ./resnet50_v1_dynamichw_fp32.om --input=./data/ --dymHW 224,224
```
...
```bash
amit profile benchmark --model resnet50_v1_dynamicshape_fp32.om --input=./data/ --dymDims actual_input_1:1,3,224,224
```
...
```bash
amit profile benchmark --model resnet50_v1_dynamicshape_fp32.om --input=./data/ --auto_set_dymdims_mode 1
```
...
```bash
amit profile benchmark --model resnet50_v1_dynamicshape_fp32.om --dymShape actual_input_1:1,3,224,224 --outputSize 10000
```
...
```bash
amit profile benchmark --model ./pth_resnet50_dymshape.om  --outputSize 100000 --auto_set_dymshape_mode 1  --input ./dymdata
```
...
```bash
amit profile benchmark --model ./pth_resnet50_dymshape.om  --outputSize 100000 --dymShape_range actual_input_1:1,3,224,224~226
```
...
+ profiler为固化到程序中的一组性能数据采集配置，生成的性能数据保存在--output参数指定的目录下的profiler文件夹内。
  ...
  该参数是通过调用amit/profile/benchmark/ais_bench/infer/__main__.py中的msprof_run_profiling函数来拉起msprof命令进行性能数据采集的。若需要修改性能数据采集参数，可根据实际情况修改msprof_run_profiling函数中的msprof_cmd参数。示例如下：
...
```bash
amit profile benchmark --model ./resnet50_v1_bs1_fp32.om --acl_json_path ./acl.json
amit profile benchmark --model /home/model/resnet50_v1.om --output ./ --dump 1
amit profile benchmark --model /home/model/resnet50_v1.om --output ./ --profiler 1
```
...
- 仅设置--output参数。示例命令及结果如下：
  ...
  ```bash
  amit profile benchmark --model ./pth_resnet50_bs1.om --output ./result
  ```
...
- 设置--input和--output参数。示例命令及结果如下：
  ...
  ```bash
  amit profile benchmark --model ./pth_resnet50_bs1.om --input ./data --output ./result
  ```
...
- 设置--output_dirname参数。示例命令及结果如下：
  ...
  ```bash
  amit profile benchmark --model ./pth_resnet50_bs1.om --output ./result --output_dirname subdir
  ```
...
- 设置--dump参数。示例命令及结果如下：
  ...
  ```bash
  amit profile benchmark --model ./pth_resnet50_bs1.om --output ./result --dump 1
  ```
...
- 设置--profiler参数。示例命令及结果如下：
  ...
  ```bash
  amit profile benchmark --model ./pth_resnet50_bs1.om --output ./result --profiler 1
  ```
...
```bash
amit profile benchmark --model ./test/testdata/bert/model/pth_bert_bs1.om --input ./random_in0.bin,random_in1.bin,random_in2.bin
```
